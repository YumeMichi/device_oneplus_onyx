...
{
    int i;
    int input_len = strlen(szBDAddr);
    int last_dest_base = (input_len/2 - 1) * 3;
    char szBDAddr_parallel[input_len + 1];
    strcpy(szBDAddr_parallel , szBDAddr);

    for (i = 0; i < input_len/2; i++) {
        szBDAddr[last_dest_base - i*3] = szBDAddr_parallel[i*2];
        szBDAddr[last_dest_base - i*3 + 1] = szBDAddr_parallel[i*2 + 1];
        szBDAddr[last_dest_base - i*3 + 2] = cSep;
    }
    szBDAddr[last_dest_base + 2] = 0;
}
